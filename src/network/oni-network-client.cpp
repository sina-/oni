...
namespace oni {
    ...
    Client::connect(const Address &address) {
        ...
        if (!mEnetServer) {
            printl("Failed to initiate connection to: " + address.host + ":" + std::to_string(address.port));
            assert(false);
        }
        ENetEvent event;
        if (enet_host_service(mEnetHost, &event, 5000) > 0 && event.type == ENET_EVENT_TYPE_CONNECT) {
            printl("Connected to: " + address.host + ":" + std::to_string(address.port));
        } else {
            printl("Failed connecting to: " + address.host + ":" + std::to_string(address.port));
            assert(false);
        }
    ...
    Client::sendMessage(std::string &&message) {
        ...
        auto data = oni::serialize<Packet_Data>(messagePacket);
    ...
    void
    Client::sendInput(const Input *input) {
        if (!input->hasData()) {
            return;
        }
        auto type = PacketType::CLIENT_INPUT;
        auto data = oni::serialize<Input>(*input);
