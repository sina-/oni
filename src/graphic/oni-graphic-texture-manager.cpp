...
namespace oni {
    namespace graphic {
        ...
        void
        TextureManager::swapTextures(component::Texture &texture) {
#if 0
            {
                auto r = FI_RGBA_RED;
                auto g = FI_RGBA_GREEN;
                ...
                for (int i = 0; i < texture.image.data.size(); ++i) {
                    texture.image.data[i] = (texture.image.data[i] + texture.image.dataPrevious[i]) / 2;
                    texture.image.dataPrevious[i] = texture.image.data[i];
                }
                auto stride = texture.image.width * 4;

                for (common::i32 y = 0; y < texture.image.height; ++y) {
                    for (common::i32 x = 0; x < texture.image.width; ++x) {
                        auto element = y * stride + x;
                        auto newR = texture.image.data[element + r];
                        auto oldR = texture.image.dataPrevious[element + r];

                        auto newG = texture.image.data[element + g];
                        auto oldG = texture.image.dataPrevious[element + g];

                        auto newB = texture.image.data[element + b];
                        auto oldB = texture.image.dataPrevious[element + b];

                        auto newA = texture.image.data[element + a];
                        auto oldA = texture.image.dataPrevious[element + a];
                        ...
                        texture.image.data[element + r] = blendR * 255;
                        texture.image.data[element + g] = blendG * 255;
                        texture.image.data[element + b] = blendB * 255;
                        texture.image.data[element + a] = blendA * 255;

                        texture.image.dataPrevious[element + r] = blendR * 255;
                        texture.image.dataPrevious[element + g] = blendG * 255;
                        texture.image.dataPrevious[element + b] = blendB * 255;
                        texture.image.dataPrevious[element + a] = blendA * 255;
                    }
                }
                bind(texture.textureID);
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texture.image.width, texture.image.height, 0, texture.format,
                             texture.type,
                             texture.image.data.data());
                unbind();
            }
...
//                                   texture.textureIDPrevious, GL_TEXTURE_2D, 0, 0, 0, 0,
//                                   texture.image.width, texture.image.height, 1);
            }
#endif

            // std::swap(texture.textureID, texture.textureIDPrevious);
        }
